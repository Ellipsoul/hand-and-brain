  ...
  wss.on('connection', (ws) => {
    ws.on('message', async (data, isBinary) => {
      ...
      if (msg.type === 'join') {
        ...
        if (!lob) {
          ws.send(JSON.stringify({ type: 'error', error: 'Lobby not found or expired' }));
          return;
        }
        ...
        if (!exists) {
          lob.players.push({ id: player.id, name: player.name, isObserver: true });
        }
      ...
      if (msg.type === 'role') {
        ...
        if (selection) {
          ...
          if (occ && occ !== sess.playerId) {
            ws.send(JSON.stringify({ type: 'error', error: 'Role already occupied' }));
            return;
          }
